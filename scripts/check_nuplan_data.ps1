...
$awsMiniPath = Join-Path $NuplanDataRoot "data\cache\mini"
...
$miniSourcePath = if (Test-Path $miniPath) { $miniPath } elseif (Test-Path $awsMiniPath) { $awsMiniPath } else { $miniPath }
$miniDbCount = if (Test-Path $miniSourcePath) { @(Get-ChildItem -Path $miniSourcePath -Filter "*.db" -File -ErrorAction SilentlyContinue).Count } else { 0 }
...
Show-Status "mini db files" ($miniDbCount -gt 0) "$miniSourcePath ($miniDbCount .db files)"
Show-Status "nuPlan mini layout" (Test-Path $miniPath) "$miniPath"
...
Write-Output ""
if (($miniDbCount -gt 0 -or $trainvalDbCount -gt 0) -and (Test-Path $miniPath) -and (Test-Path $mapsJson) -and ($gpkgFiles.Count -ge 4)) {
    ...
    Write-Output "  scripts\download_nuplan_mini.ps1 -DatasetRoot D:\nuplan-data\dataset"
    Write-Output "  scripts\check_nuplan_data.ps1 -NuplanDataRoot D:\nuplan-data\dataset -NuplanMapsRoot D:\nuplan-data\dataset\maps -NuplanExpRoot D:\nuplan-data\exp"
}
